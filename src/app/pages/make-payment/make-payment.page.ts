...
export class MakePaymentPage implements OnInit {
  ...
  packPrice = Number(localStorage.getItem("packPrice"));
  noOfPacks = 1;
  ...
  ngOnInit() {
    ...
    this.finalTotal =
      Number(this.totalPayment) + Number(this.serviceCharge) + Number(this.VAT);
  ...
  ionViewWillEnter() {
    ...
    // this.packPrice = this.totalItems.filter.length * +this.packPrice;
    ...
    this.noOfPacks =
      this.items.filter((item) => item.hasPack).length < 1
        ? 1
        : this.items.filter((item) => item.hasPack).length;
    this.packPrice *= Number(this.noOfPacks);

    this.finalTotal += Number(this.packPrice);
    if (this.discount >= 0) {
      this.discountTotal();
    }
  }
